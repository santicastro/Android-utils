...
package es.skastro.android.util.bluetooth;
...
public class BluetoothService {
    ...
    public static final int CONFIRMATION_RECEIVED = 6;
    ...
    public boolean writeAndWait(byte[] out) {
        // Create temporary object
        ConnectedThread r;
        // Synchronize a copy of the ConnectedThread
        synchronized (this) {
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    while (mmInStream.available() > 0 && offset < 1024) {
        ...
        private int WAIT_MILLIS = 10;

        public boolean writeAndWait(byte[] buffer) {
            waitingForResponse = true;
            responseReceived = false;
            write(buffer);
            for (int i = 0; i < TIMEOUT; i += WAIT_MILLIS) {
                if (responseReceived) {
